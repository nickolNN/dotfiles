-- Send-under-cursor handlers: function context, all diagnostics, word under cursor.

local fn_name_mod = require("agent-integration.send_under_cursor.fn_name")

local function _format_diagnostic_part(d)
	local parts = {}
	local level = vim.diagnostic.severity[d.severity] or "HINT"
	table.insert(parts, level .. " line " .. (d.lnum + 1) .. " col " .. (d.col + 1))
	if d.code then
		table.insert(parts, "[" .. d.code .. "]")
	end
	if d.message then
		table.insert(parts, d.message)
	end
	if d.source then
		table.insert(parts, "(source: " .. d.source .. ")")
	end
	return table.concat(parts, " | ")
end

local function get_formatted_diagnostics(line_number_filter)
	local diags = vim.diagnostic.get(0, {
		severity = { min = vim.diagnostic.severity.HINT },
	})
	if #diags == 0 then
		return nil
	end

	local parts = {}
	for _, d in ipairs(diags) do
		if line_number_filter == nil or d.lnum == line_number_filter then
			table.insert(parts, _format_diagnostic_part(d))
		end
	end

	return #parts > 0 and parts or nil
end

return function(ctx, config)
	local display = config.display_name

	local function send_under_cursor(opts)
		local line_number = ctx.get_cursor_line()
		local fn = fn_name_mod.fn_name_under_cursor() or "<none>"
		local relative_path = ctx.get_relative_path()
		local text_to_send = "@" .. relative_path .. " line " .. line_number .. " (function: " .. fn .. ")"
		local diag_parts = get_formatted_diagnostics(line_number - 1)
		if diag_parts then
			text_to_send = text_to_send .. " [errors/warnings: " .. table.concat(diag_parts, "; ") .. "]"
		end
		ctx.send(
			text_to_send,
			"Function context added to " .. display .. ": " .. fn,
			{ focused = opts and opts.focused },
			relative_path
		)
	end

	local function send_all_diagnostics(opts)
		local relative_path = ctx.get_relative_path()
		local diag_parts = get_formatted_diagnostics(nil)
		if not diag_parts then
			vim.notify("No diagnostics found in current buffer", vim.log.levels.INFO)
			return
		end

		local text_to_send = "fix all problems in @" .. relative_path .. ":\n" .. table.concat(diag_parts, "\n")
		ctx.send(
			text_to_send,
			"All diagnostics sent to " .. display .. " for: " .. relative_path,
			{ focused = opts and opts.focused },
			relative_path
		)
	end

	local function send_word_under_cursor(opts)
		local line_number = ctx.get_cursor_line()
		local word = vim.fn.expand("<cWORD>")
		if not word or string.match(word, "^%s*$") then
			vim.notify("No word under cursor", vim.log.levels.WARN)
			return
		end
		local relative_path = ctx.get_relative_path()
		local text_to_send = "@" .. relative_path .. " line " .. line_number .. " " .. word
		ctx.send(
			text_to_send,
			"Word + context sent to " .. display .. ": " .. word,
			{ focused = opts and opts.focused },
			relative_path
		)
	end

	return {
		send_under_cursor = send_under_cursor,
		send_all_diagnostics = send_all_diagnostics,
		send_word_under_cursor = send_word_under_cursor,
	}
end
