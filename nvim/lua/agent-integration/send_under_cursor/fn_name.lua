-- Function-name detection: treesitter first, regex fallback.
...
local function fn_name_under_cursor()
	...
	if name then
		_set_cached_fn_name(name, buf, line, col)
		return name
	end
	...
	if fn and fn ~= "" then
		_set_cached_fn_name(fn, buf, line, col)
		return fn
	end
	local cword = vim.fn.expand("<cword>")
	_set_cached_fn_name(cword, buf, line, col)
	return cword
end
